...
export default function LoginPage() {
    ...
    const handleSubmission = async (e) => {
        ...
        try {
            ...
            if (!res.ok) {
                const errorData = await res.json(); // Await the response data
                setError(errorData.message || "An error occurred.");
                setStatus("Sign up"); // Reset status
                return;
            }
    ...
    return (
        <main className="w-full h-screen flex flex-col items-center justify-center px-4">
            <div className="max-w-sm w-full text-gray-600 space-y-5">
                ...
                <p className="text-center">Don't have an account? <a href="#" className="font-medium text-indigo-600 hover:text-indigo-500">Sign up</a></p>
            </div>
        </main>
    );
}
